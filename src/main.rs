    ...
    Term::Binary(bin) => {
      ...
      if let (Val::Int(a), Val::Int(b)) = (lhs.clone(), rhs.clone()) {
        ...
        for c in a.chars().into_iter() {
          if c.is_numeric() {
            number = c.to_string().parse::<i32>().unwrap();
          }
        }
      ...
      } else if let (Val::Int(a), Val::Str(b)) = (lhs.clone(), rhs) {
        return Val::Str(match bin.op {
          BinaryOp::Add | BinaryOp::Sub | BinaryOp::Mul => {
            format!("{}{}", b, a)
          },
        ...
        panic!("not is a number or string");
      }
    },
    Term::If(i) => {
      let condition = eval(i.condition);
      match condition {
        Val::Bool(true) => eval(i.then),
        Val::Bool(false) => eval(i.otherwise),
        _ => panic!("error in condition"),
      }
    },
    Term::Bool(boolean) => Val::Bool(boolean.value),
    Term::Tuple(tp) => {
      let tp_first = eval(tp.first);
      let tp_second = eval(tp.second);
